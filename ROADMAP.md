...
- [x] M7.2 — Uniform provider/model catalog: the `selune` crate (Catwalk format) provides `Provider`/`Model`/`TokenUsage`-independent data, `shuvarie-core::catalog` loads providers (fetch with embedded fallback) and derives `estimate_cost`/`context_length`/`enrich`/`requires_api_key`/endpoint behavior, and `shuvarie-llm` owns the shared `TokenUsage`/`ModelInfo` types. Deferred follow-up: user-configurable rate overrides, and surfacing deployment-tier variants in the ModelPicker.
