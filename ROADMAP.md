...
- `shuvarie-core`, `shuvarie-db`, `shuvarie-llm`, `selune`, and `shuvarie-highlight` wired as dependencies of the root binary, with real crate roots and typed error enums (`CoreError`, `DbError`, `LlmError` via `thiserror`).
...
- `selune` (git submodule at `vendor/selune`) is the uniform source of provider + model info, in the Catwalk format: `Provider`/`Model`/`ModelOptions`/`ProviderType`, a blocking `Client` fetching a hosted catalog JSON (`CATALOG_URL` / `DEFAULT_URL`) plus `embedded::all()` offline configs. Provider identity is a catalog `id` string. `shuvarie-core::catalog` loads providers (fetch with embedded fallback) and derives pricing/context/requires-key/endpoint behavior; `shuvarie-llm` owns the shared `TokenUsage`/`Model` types (re-exported from rig).
- `shuvarie-llm` exposes a `ProviderClient` that builds the right `rig` client per kind, an async `list_models` returning `rig::model::Model` (re-exported as `Model`), and a streaming `stream` (via `rig::streaming::StreamingChat`) yielding a portable `StreamItem` stream (`Delta`/`ToolStart`/`ToolResult`/`Done`/`Error`). Rig types stay out of `shuvarie-core`; `ChatMsg`/`Role` are the portable message types, and `TokenUsage`/`ModelInfo` are re-exported from rig (`rig::completion::Usage`, `rig::model::Model`).
- `shuvarie-core` defines the config schema (`Config`, `UiPrefs`, `AgentConfig`) and the connections schema (`Connections`, `ProviderConfig`), loads/saves `~/.config/shuvarie/config.kdl` and `~/.config/shuvarie/connections.kdl` via the `dirs` crate, and exposes `has_connected_providers()`. An in-memory `Session` struct holds messages + token/cost totals (input/output/reasoning/cache + cost, accumulated from real `Usage` on each completed reply) plus persisted `id`/`title` tracking.
...
- Responses stream token-by-token into the in-flight assistant message (status row "streaming…"); `Ctrl+C` mid-stream aborts and keeps the partial reply as an interrupted assistant turn in the chat (`Ctrl+C` while idle opens the confirm-quit dialog). Session Context panel in the sidebar shows accumulated input/output/reasoning/cache tokens and an estimated cost (per-model rates from the `selune` catalog via `shuvarie-core::catalog::estimate_cost`, first-model fallback for unknown ids).
- Model listing is enriched from `shuvarie-core::catalog` (`enrich` fills missing runtime context lengths), so the ModelPicker shows a context label and the sidebar's used-context percentage works with real data. The ModelPicker offers the raw query as a selectable first row when no model matches, so arbitrary model ids can be submitted.
...
- Skills system: `shuvarie-core::skills` scans workspace `.agents/skills/`, global `~/.config/shuvarie/skills/`, and `[skills] dirs` for `SKILL.md` files, parsing their YAML front-matter. Skills surface in the sidebar Skills panel and are listed in the agent preamble so the agent can consult a skill's `SKILL.md` when relevant. See the M7+ bullet below.
...
- [x] Add `serde` + `kdl` to `shuvarie-core` for config loading.
...
- [x] In `shuvarie-core`: define the config file schema (`~/.config/shuvarie/config.kdl` — UI prefs, `embedding`, `agent` turn budgets) and the connections file schema (`~/.config/shuvarie/connections.kdl` — providers with API key + base URL, active provider, active model).
...
- [x] Keep provider config in the KDL file — do not migrate secrets into the database.
...
- [x] M7.1 — Tool calling (file read/write/edit, shell execution, list, grep) via a portable `Tool` trait over rig's `DynamicTool`, with a multi-turn agent loop, an agent preamble, inline tool-activity rendering in the chat pane, and an approval gate on file tools (outside-workspace/hidden-path prompts). The turn budget is configurable via `config.kdl` `agent` `max-turns` (`0` = unlimited, the default). Deferred follow-ups: tool-output token counting.
- [x] M7.2 — Uniform provider/model catalog: the `selune` crate (Catwalk format) provides `Provider`/`Model`/`TokenUsage`-independent data, `shuvarie-core::catalog` loads providers (fetch with embedded fallback) and derives `estimate_cost`/`context_length`/`enrich`/`requires_api_key`/endpoint behavior, and `shuvarie-llm` owns the shared `TokenUsage`/`Model` types (re-exported from rig). Deferred follow-up: user-configurable rate overrides, and surfacing deployment-tier variants in the ModelPicker.
- [x] M7.3 — Multi-agent orchestration and task decomposition: an always-active orchestrator agent (the main session agent) exposes three worker agents as tools — `explore_workspace` (list/read/grep), `run_tests` (build/test/lint), and `edit_files` (read/write/edit) — each running its own LLM agent loop over a narrowed tool set with a role-specific preamble. The worker turn budget is configurable via `config.kdl` `agent` `worker-max-turns` (`0` = unlimited, the default). Workers surface inline in the chat pane: the delegate call renders with a `❖` marker and the worker's own tool calls render indented beneath it, tagged with the owning worker. Worker token usage accumulates into the session Context totals alongside the manager's. Deferred follow-up: per-worker model choice and deeper nesting (workers calling workers).
...
- [x] M9 — LSP integration: a new `shuvarie-lsp` crate wraps `async-lsp` to spawn per-language LSP servers (`rust-analyzer`, `gopls`, `clangd`, `pyright`, `typescript-language-server`, `zls`, `lua-language-server`, `solargraph`) as child processes over stdio, with a built-in registry probed lazily on PATH and `server <lang>` overrides in `config.kdl`. The core task owns an `LspManager` (in `Arc<Mutex<..>>`) shared with the file tools (`write_file`/`edit_file` auto-start the matching server + `didChange`, gated by `the `lsp` `enabled` flag + per-spec `auto-start`; `read_file` is a pure read with no LSP side effects) and the new `lsp` agent tool (`start`/`stop`/`restart`/`list` with `name` as exact id or substring filter + `all` flag, and `analyze` with a `path` — file or directory, defaulting to the workspace root — that starts the matching server if needed, opens the target file(s), and returns the published diagnostics). `publishDiagnostics` are pumped every 500 ms into `Event::LspDiagnostics` and rendered inline beneath the diff in the chat pane; `Event::LspStatus` drives the sidebar LSP panel (running servers with `✓`/`⟳`/`✗` markers + per-server diagnostic counts; "no servers" / "disabled" placeholders when idle). Deferred follow-ups: incremental `didChange` sync (currently full document), code-action/quick-fix surface, and multi-root workspaces.
- [x] Skills system (real skills panel instead of inactive placeholder): a new `shuvarie-core::skills` module scans skill directories (workspace `.agents/skills/`, the global `~/.config/shuvarie/skills/`, plus any the `skills` `dirs` in `config.kdl`) for `SKILL.md` files and parses their YAML front-matter (`name`, `description`, `metadata.category`, `metadata.tags`). Skills load at core-task startup and surface in the sidebar Skills panel (name + description per skill; "none" placeholder when empty). The agent preamble lists available skills (name + description + path) so the agent can `read_file` a skill's `SKILL.md` when a task matches its domain. `[skills] enabled` (default true) gates the system.
...
The OpenCode codebase (`./opencode/packages/opencode/src/tool/`) defines a rich built-in tool roster. Shuvarie already has nine tools (`read_file`, `write_file`, `edit_file`, `run_shell`, `list_dir`, `grep`, `glob`, `lsp`, `todo` — `tools.rs`), three worker agents (`explore_workspace`, `run_tests`, `edit_files` — `agents.rs`), an approval gate, inline diff review, and a skills system. This milestone closes the gap by replicating the remaining OpenCode tools and behaviors that fit Shuvarie's architecture, and by hardening the ones we have.
...
Mapping to Shuvarie today: `shell`≈`run_shell`, `read`≈`read_file`, `edit`≈`edit_file`, `write`≈`write_file`, `grep`≈`grep`, `webfetch`≈`webfetch`, `lsp`≈`lsp`, `task`≈`explore_workspace`/`run_tests`/`edit_files`, `skill`≈skills system, `todo`≈`todo`, `question`≈`question`. Missing or underdeveloped: `websearch`, `apply_patch`, `plan`, `invalid`.

- [x] **M10.1 — Glob tool.** Add a `glob` agent tool (`glob.rs`) mirroring OpenCode's `glob.ts`: recursive pattern matching over the workspace, returning matched paths with a count cap, honoring `.gitignore`/hidden-path exclusions consistent with the existing `walk_dir`/`resolve_checked` logic, and gated by the same `ApprovalGate` when the pattern resolves outside the workspace. Reuse a glob crate (e.g. `globset`) rather than hand-rolling. Deferred: fuzzy match mode and per-directory depth limits.
- [x] **M10.2 — Todo tool.** Add a `todo` agent tool (`tools.rs`) mirroring OpenCode's `todo.ts`: a small session-scoped task list the agent can create/update/complete, rendered inline in the chat pane (distinct `▸`/`☑`/`☐` markers at each update's timeline position), persisted per-session in the DB (`todos` table, full-list replace on each call), surfaced in the sidebar Todo panel (`done/total` count) and as a top bar of in-progress items under the title. The new list is attached to the `ToolContext` as a `TodoUpdate` (captured by a `TodoHook` in `shuvarie-llm`, mirroring `FileChangeHook`) and forwarded through `StreamItem::ToolResult` → `Event::ToolFinished` → the TUI. Deferred: reordering and priority.
- [x] **M10.3 — WebFetch tool.** Add a `webfetch` agent tool (`tools.rs`, mirroring OpenCode's `webfetch.ts`): fetch a URL and return its content as text/markdown/html, with a 5 MB download cap (checked on `content-length` and while streaming), Chrome UA + q-weighted `Accept` headers, a `cf-mitigated: challenge` 403 retry with the honest `shuvarie` UA, and output truncation via the existing `[context].tool_output_max_chars` cap with the `… (output truncated: …)` marker. HTML is detected by `content-type` (with a URL-extension fallback when the header is missing) and converted via `html-to-markdown-rs` (GFM tables, script/style stripped, images skipped, Plain format for `text`); non-HTML content (JSON, XML, source) returns as-is; image responses are rejected. Every fetch is gated through the `ApprovalGate` with a new `ApprovalReason::Network` (URL shown in the existing `ApprovalPrompt` overlay; session-level "Always approve" covers all later fetches). Registered in `all_tools` and `read_tools`. Deferred: `websearch` (provider-gated) and parallel fetching.
- [x] **M10.4 — Question tool.** Add a `question` agent tool (`crates/core/src/question.rs` + the `Question` tool in `tools.rs`) mirroring OpenCode's `question.ts`: the agent asks one or more questions (`question`/`header`/`options` with `multiple`/`custom` flags); a `QuestionGate` (mpsc + oneshot, cloned from the `ApprovalGate` pattern) blocks the tool until the user answers. The TUI's `QuestionUI` (`src/tui/question.rs`) replaces the session text area while a question is pending (`SessionScreen.question` gates key routing and the `input_area` render slot; the `TextArea` model is untouched, so the draft returns intact when the question closes). Single-question single-select submits on pick; multi-question flows use per-question tabs (←/→/Tab, `1..9` shortcuts) plus a Confirm review; multi-select toggles `[x]`; `custom` opens an inline "Type your own answer" entry. Esc dismisses → the tool fails with "The user dismissed this question". Pending questions are invalidated on `CancelStream`/`NewSession`/`LoadSession`. Completed calls render as a Q&A block in the chat pane (question + chosen answers parsed from the persisted tool args/output), surviving session reload.
