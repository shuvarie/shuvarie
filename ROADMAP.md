...
- [x] Diff review + approval prompts (M7+): `edit_file`/`write_file` attach a `FileChange` (`Edit { path, diff }` via `similar::TextDiff`, or `Write { path, content }`) rendered inline in the chat pane (line-number gutter, `…` ellipsis between hunks; new files show content with line numbers). File tools prompt for approval on outside-workspace/hidden paths via an `ApprovalGate` + `ApprovalPrompt` overlay (Approve / Always approve / Deny); `run_command` is never gated.
