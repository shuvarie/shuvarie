...
Mapping to Shuvarie today: `shell`≈`run_shell`, `read`≈`read_file`, `edit`≈`edit_file`, `write`≈`write_file`, `grep`≈`grep`, `lsp`≈`lsp`, `task`≈`explore_workspace`/`run_tests`/`edit_files`, `skill`≈skills system, `todo`≈`todo`. Missing or underdeveloped: `webfetch`, `websearch`, `apply_patch`, `question`, `plan`, `invalid`.
