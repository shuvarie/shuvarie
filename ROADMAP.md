...
- Approval prompts (M7+): file tools (`read_file`, `write_file`, `edit_file`, `list_dir`, `grep`) prompt for approval when the target path is outside the workspace or under a hidden path; `run_shell` prompts only when its `cwd` argument resolves outside the workspace. The `ApprovalPrompt` overlay offers Approve / Always approve / Deny (`←`/`→` focus, Enter/Esc); "Always approve" auto-approves for the rest of the session.
