...
- `shuvarie-core` defines the config schema (`Config`, `UiPrefs`) and the connections schema (`Connections`, `ProviderConfig`), loads/saves `~/.config/shuvarie/config.toml` and `~/.config/shuvarie/connections.toml` via the `dirs` crate, and exposes `has_connected_providers()`. An in-memory `Session` struct holds messages + token/cost totals (input/output/reasoning/cache + cost, accumulated from real `Usage` on each completed reply) plus persisted `id`/`title` tracking.
...
- [x] In `shuvarie-core`: define the config file schema (`~/.config/shuvarie/config.toml` — UI prefs, `[embedding]`) and the connections file schema (`~/.config/shuvarie/connections.toml` — providers with API key + base URL, active provider, active model).
...
- [x] M7.1 — Tool calling (file read/write/edit, shell execution, list, grep) via a portable `Tool` trait over rig's `DynamicTool`, with a multi-turn agent loop (`max_turns` 20), an agent preamble, inline tool-activity rendering in the chat pane, and an approval gate on file tools (outside-workspace/hidden-path prompts). Deferred follow-ups: tool-output token counting, configurable turn budget.
...
- [x] Diff review + approval prompts (M7+): `edit_file`/`write_file` attach a `FileChange` (`Edit { path, diff, original, new }` via `similar::TextDiff`, or `Write { path, content, original }`) rendered inline in the chat pane (line-number gutter, `…` ellipsis between hunks; new files show content with line numbers). File tools prompt for approval on outside-workspace/hidden paths via an `ApprovalGate` + `ApprovalPrompt` overlay (Approve / Always approve / Deny); `run_command` is never gated. The `original`/`new` content enables undo/redo of file changes.
- [x] M8 — Persistent tool activity + reasoning + undo/redo/replay/resume: tool call records (name, args, output, ok, worker, file-change JSON, original/new content) are persisted in a `tool_calls` table keyed to the assistant message; reasoning/thinking text is captured from rig's `StreamedAssistantContent::Reasoning`/`ReasoningDelta` and stored in `messages.reasoning`; `interrupted: bool` marks turns cancelled by the user or aborted by errors. On session reload, `Session::from_stored` reconstructs tool records + reasoning so the chat pane shows full tool activity and diffs. `UndoLastTurn` reverts the last user+assistant pair + file changes and moves them to a DB-backed `undo_log` (multi-level, survives reload); `Redo` restores from `undo_log`; `Replay` = undo + re-send the last user message; `Resume` deletes the interrupted assistant message + re-streams from the last user message (clearing partial text). All four are accessible via `Ctrl+M` command menu (Resume only when interrupted).
