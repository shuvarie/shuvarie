...
When adding a feature: put domain logic in `shuvarie-core`, LLM/SDK glue in `shuvarie-llm`, storage/ORM glue in `shuvarie-db`, markdown/code highlighting in `shuvarie-highlight`, and only rendering + input dispatch in the root binary.
