...
- **Config** (`shuvarie-core::config`) — layered chain `$cwd/shuvarie.kdl` → `$cwd/.shuvarie/config.kdl` → global `~/.config/shuvarie/config.kdl`, merged per top-level section (except `lsp.servers`, key-by-key); hand-rolled KDL parse/serialize in `config_kdl.rs` (same shape as `connections_kdl`, shared span/error helpers in `kdl_util`), with default-on flags stored inverted (`disabled #true` = off, omitted = on) and values equal to their section default omitted from the saved file. UI/agent/context/lsp/skills/retry/shell preferences live here; no secrets. `shell.path` selects the executable `run_shell` uses (absolute/relative path or a `PATH` name); when it is not found at startup the platform default is used and a `ShellWarning` popup is shown.
...
- **Project context files** — the preamble loads the global `~/.config/shuvarie/AGENTS.md`, then the `AGENTS.md`/`CLAUDE.md` (an `AGENTS.override.md` replaces both) of every ancestor directory up to the filesystem root — one file per directory, outermost first — plus `.shuvarie/context/*` (`context.rs`). Re-read per request so edits apply without a restart; announced once per session via `Event::ContextLoaded`.
- **Sidebar context display** — every completed LLM request (main stream + workers) emits `StreamItem::Usage` → additive `Event::UsageUpdate`; each turn end emits authoritative `Event::UsageSnapshot` (session cumulative totals) that *replaces* client-side totals (so cancelled/retried turns self-heal). The context window comes from the Selune catalog (`catalog_context_length` in `src/tui/app.rs`); the window line shows the latest main-request context footprint (`UsageUpdate.context_tokens`, from `shuvarie_llm::context_footprint`; workers excluded since they run separate conversations) as % of it — bare window size while no footprint is known (fresh session, during compaction). Below the window line the sidebar (and the collapsed status line) show the latest main request's read tokens (`R…`, `shuvarie_llm::read_tokens`) and cache-hit percentage (`CH…%`, its `cached_input_tokens` over the read tokens); each half is hidden while unavailable. On session load (and undo/redo restores) the block re-seeds from `Session.last_usage`, which `Session::from_stored` derives from the message rows' persisted main-request usage (the `request_json` column, falling back to the row's usage columns); `Reset`/`CompactionStarted` clear it.
...
- Segments (`session/segment.rs`) are chunked: each segment is a list of `BodyChunk`s — `Fixed` (always-resident lines) and `Sliced` (a `BodySource` projection — output rows, numbered file content, diff rows, or `Lines` (shared pre-rendered markdown) — whose wrapped-row counts are precomputed and whose lines materialize only when painted, and only for the rows intersecting the viewport). Long tool bodies / diffs / expanded reasoning / markdown replies render as sliced chunks. Wrap counting and slicing rely on ratatui wrapping each source line independently — per-line `Paragraph::line_count` equals the line's contribution in a whole-body paragraph, so windowed paint is bit-identical (tests assert this).
