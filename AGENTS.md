...
- `./crates/highlight/` (`shuvarie-highlight`) — markdown + code highlighting for the chat pane; emits unwrapped ratatui `Line`s (wrapping stays with the TUI `Paragraph`); `render_pass` additionally reports safe top-level block boundaries so streaming blocks re-render only their open tail.
