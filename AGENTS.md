...
- `shuvarie-core` implements config load/save, `has_connected_providers()`, the `Command`/`Event` enums (including `CancelStream`, stream events, `ToolStarted`/`ToolFinished`/`WorkerStarted`/`WorkerFinished`), the in-memory `Session` struct (with `id`/`title` tracking), the persistence hooks into `shuvarie-db` (session row created lazily on first message; user message persisted on send, assistant message + real usage on stream done; most-recent session loaded on startup), the core task (`run`) that orchestrates provider/model listing and streaming chat (M5, M6), the six agent tools (`tools.rs`, M7.1: read/write/edit files, run commands, list dirs, grep) with a workspace-root escape guard, and the worker-agent roster (`agents.rs`, M7.3).
