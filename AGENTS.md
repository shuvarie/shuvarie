...
- `./crates/llm/` (`shuvarie-llm`) — Thin wrapper over `rig`: `ProviderClient` builder, model listing (via rig's `ModelListingClient`), streaming agent chat (`stream` — multi-turn with tools; `run_worker` — single-shot worker agent loop), the portable `Tool` trait + `ToolDefinition` + `ToolOutput` (the boundary types `shuvarie-core` implements; a tool returns `ToolOutput { text, file_change }` where `file_change` is an optional `FileChange` — `Edit { path, diff }` or `Write { path, content }` — and rig `DynamicTool`s are built inside `shuvarie-llm`, which forwards the `file_change` through a side channel into the emitted `StreamItem::ToolResult`), `WorkerAgent` (an agent-as-tool whose `call` runs its own `stream_chat` loop over a narrowed tool set, forwarding its tool activity through an mpsc channel merged into the manager stream), `ChatMsg`/`Role` message types, and rig→catalog conversions (`model_info_from_rig`, `token_usage_from_rig`). Depends on `shuvarie-catalog` for the shared types; no pricing/estimation lives here. No TUI concerns.
...
- **Database** (`.shuvarie/data.db` in the working directory — Turso embedded SQLite + Toasty ORM): chat sessions and message history. Provider config stays in the TOML files regardless.
...
- `shuvarie-db` implements the Toasty models (`Session`, `Message`, `MessageEmbedding`), the `Store` wrapper (open/apply embedded migrations, list/load/create/append/delete, most-recent query, BM25 full-text `search_messages` over the FTS index, `upsert_embedding`/`messages_missing_embeddings`/`semantic_search` over vector32 blobs) and the `migrate` bin (toasty-cli) for managing `crates/db/toasty/` migration files (M6, M7.5, M7.6).
- `shuvarie-core` implements config + connections load/save, `has_connected_providers()`, the `Command`/`Event` enums (including `CancelStream`, stream events, `ToolStarted`/`ToolFinished`/`WorkerStarted`/`WorkerFinished`, `ContextLoaded`, `SearchHistory`/`SearchResults`/`SearchError`, `ApprovalRequest`/`ApproveTool`), the in-memory `Session` struct (with `id`/`title` tracking), the persistence hooks into `shuvarie-db` (session row created lazily on first message; user message persisted on send, assistant message + real usage on stream done; most-recent session loaded on startup), the core task (`run`) that orchestrates provider/model listing and streaming chat (M5, M6), the six agent tools (`tools.rs`, M7.1: read/write/edit files, run commands, list dirs, grep) with an approval gate (outside-workspace/hidden-path prompts) and `compute_diff` (via `similar::TextDiff`), the worker-agent roster (`agents.rs`, M7.3), project-aware context loading (`context.rs`, M7.4: on each `SendMessage` the workspace-root `AGENTS.md` and `.shuvarie/context/` files load — sorted, 64 KB/file + 256 KB total caps, binary skipped — appended to the agent preamble as `=== <path> ===` sections, announced to the TUI via `Event::ContextLoaded`), the embedding subsystem (`embeddings.rs`, M7.6: `setup` resolves the `[embedding]` config to a client/model/dims, on-write `index_message` spawns fire-and-forget, startup `backfill` batches 64, `rrf_merge` fuses FTS+semantic hit lists), and hybrid history search dispatch (`Command::SearchHistory` → FTS `Store::search_messages` emitted immediately, then a 300 ms-debounced query embed + `semantic_search` merged via RRF, capped at 50 hits, M7.5/M7.6).
...
- `shuvarie-llm` streaming runs a multi-turn agent loop (`max_turns` 20) with an agent preamble supplied by the caller (the fixed orchestrator preamble plus the loaded project context, in `shuvarie-core`); `StreamItem` carries `Delta`/`ToolStart`/`ToolResult`/`WorkerStart`/`WorkerResult`/`Done`/`Error`. A `Done` with empty text after tool calls means a tool-output-only turn. Tool errors surface as JSON error objects so the TUI marks them failed while the model still sees the message.
- Diff review + approval prompts (M7+): `shuvarie-llm`'s `Tool` trait returns `ToolOutput { text, file_change }`; `shuvarie-core`'s `edit_file`/`write_file` attach a `FileChange` (`Edit { path, diff }` computed via `similar::TextDiff`, or `Write { path, content }`), forwarded through `StreamItem::ToolResult.file_change` → `Event::ToolFinished.file_change` → the TUI's inline diff renderer (line-number gutter + `…` ellipsis between hunks). File tools gate on outside-workspace/hidden paths via an `ApprovalGate` (mpsc + `oneshot`); the core `run` loop `tokio::select!`s the approval channel and emits `Event::ApprovalRequest`, answered by the TUI's `ApprovalPrompt` overlay (`Command::ApproveTool { id, approved, always }`).
