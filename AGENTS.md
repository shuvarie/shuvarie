...
- `./vendor/selune/` (`selune`, git submodule) — uniform source of provider + model info (Catwalk format): wire types, a hosted catalog client, offline embedded configs. Provider identity is a catalog `id` string; `shuvarie-core::catalog` derives pricing/context/key requirements from it.
...
- **Connections** (`connections.kdl`, same module) — providers keyed by `id` (`kind` = selune catalog id) plus the active provider/model; hand-rolled KDL in `connections_kdl`. Secrets live here — never in the database or `config.kdl`.
