...
- `shuvarie-core` implements config load/save, `has_connected_providers()`, the `Command`/`Event` enums (including `CancelStream`, stream events, `ToolStarted`/`ToolFinished`/`WorkerStarted`/`WorkerFinished`, `ContextLoaded`), the in-memory `Session` struct (with `id`/`title` tracking), the persistence hooks into `shuvarie-db` (session row created lazily on first message; user message persisted on send, assistant message + real usage on stream done; most-recent session loaded on startup), the core task (`run`) that orchestrates provider/model listing and streaming chat (M5, M6), the six agent tools (`tools.rs`, M7.1: read/write/edit files, run commands, list dirs, grep) with a workspace-root escape guard, the worker-agent roster (`agents.rs`, M7.3), and project-aware context loading (`context.rs`, M7.4: on each `SendMessage` the workspace-root `AGENTS.md` and `.shuvarie/context/` files load — sorted, 64 KB/file + 256 KB total caps, binary skipped — appended to the agent preamble as `=== <path> ===` sections, announced to the TUI via `Event::ContextLoaded`).
