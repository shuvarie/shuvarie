...
- `./crates/llm/` (`shuvarie-llm`) — Thin wrapper over `rig`: `ProviderClient` builder, model listing (via rig's `ModelListingClient`), streaming agent chat (`stream` — multi-turn with tools), the portable `Tool` trait + `ToolDefinition` (the boundary type `shuvarie-core` implements; rig `DynamicTool`s are built inside `shuvarie-llm`), `ChatMsg`/`Role` message types, and rig→catalog conversions (`model_info_from_rig`, `token_usage_from_rig`). Depends on `shuvarie-catalog` for the shared types; no pricing/estimation lives here. No TUI concerns.
