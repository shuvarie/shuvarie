...
- **Undo / redo / replay / resume** — undo moves the last user+assistant turn into `undo_log` and reverts file changes; redo restores it; replay = undo + re-send; resume = re-stream an interrupted turn. `run_shell` effects are never reverted.
